...
def test_get_utilization_success(medicare_client):
    mock_response = Mock()
    mock_response.json.return_value = {"npi": "1234567890", "year": "2022", "utilization": 100}
    mock_response.raise_for_status.return_value = None

    with patch.object(medicare_client.client, "get", return_value=mock_response) as mock_get:
        result = medicare_client.get_utilization("1234567890", year="2022")
...
def test_get_utilization_http_error(medicare_client):
    mock_response = Mock()
    mock_response.raise_for_status.side_effect = Exception("HTTP Error")

    with patch.object(medicare_client.client, "get", return_value=mock_response):
        with pytest.raises(Exception, match="HTTP Error"):
            medicare_client.get_utilization("1234567890", year="2022")

@pytest.fixture
def physician_client():
    return CMSPhysicianCompareClient()


def test_search_providers_success(physician_client):
    fake_response_data = {
        "results": [
    ...
    with patch.object(httpx.Client, "get") as mock_get:
        ...
        result = physician_client.search_providers(specialty="Pulmonary Disease", state="CA", limit=2)
...
def test_search_providers_with_state(physician_client):
    with patch.object(httpx.Client, "get") as mock_get:
        mock_resp = MagicMock()
        ...
        result = physician_client.search_providers(specialty="Cardiology", state="TX", limit=1)
...
def test_search_providers_http_error(physician_client):
    with patch.object(httpx.Client, "get") as mock_get:
        mock_resp = MagicMock()
        ...
        with pytest.raises(httpx.HTTPStatusError):
            physician_client.search_providers("Pulmonary Disease")
